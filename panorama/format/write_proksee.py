...
def write_legend_items(legend_data: dict, features: List[str], sources: List[str]):
    ...
    legend_data["items"] = [
        {
            "name": "CDS",
            "swatchColor": f"rgba({','.join(map(str, colors.pop(1)))},0.5)",
            "decoration": "arrow",
        },
        ...
        {
            "name": "cloud",
            "swatchColor": f"rgba({','.join(map(str, colors.pop(17)))},1)",
            "decoration": "arc",
        },
    ]
    if "rgp" in features or "all" in features:
        (
            legend_data["items"].append(
                {
                    "name": "RGP",
                    "swatchColor": f"rgba({','.join(map(str, colors.pop(6)))}, 1)",
                    "decoration": "arc",
                }
            ),
        )
    if "spots" in features or "all" in features:
        legend_data["items"].append(
            {
                "name": "Spot",
                "swatchColor": f"rgba({','.join(map(str, colors.pop(5)))}, 1)",
                "decoration": "arc",
            }
        )
    if "modules" in features or "all" in features:
        legend_data["items"].append(
            {
                "name": "Module",
                "swatchColor": f"rgba({','.join(map(str, colors.pop(3)))},1)",
                "decoration": "arc",
            }
        )
...
def write_systems(
    ...
    def write_systems_by_source(
        ...
        for sys in tqdm(
            pangenome.get_system_by_source(source),
            unit="System",
            disable=True,
                ...
                for gf in gf_intersection:
                    for gene in gf2genes[gf.name]:
                        ...
                        if sys.source in gene.family.sources:
                            annotations = [
                                annot.name
                                for annot in gene.family.get_source(sys.source)
                                if annot.name in families_names
                            ]

                        systems_source_data_list.append(
                            {
                                "name": str(sys.ID),
                                "presence": "System",
                                "start": gene.start,
