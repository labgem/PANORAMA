...
def utils_hmm_list(test_data_path: Path, tmp_path_factory):
    ...
    outdir = (
        tmp_path_factory.mktemp("utils") / "hmm_utils"
    )  # Create temp dir using factory
    ...
    with open(utils_hmm_list, "r") as f:
        ...
        for line in lines[1:]:
            ...
            assert Path(path).exists(), (
                f"HMM file {path} in {utils_hmm_list} does not exist."
            )
...
def utils_model_list(test_data_path: Path, tmp_path_factory):
    ...
    outdir = (
        tmp_path_factory.mktemp("utils") / "model_utils"
    )  # Create temp dir using factory
...
def test_model_list(utils_model_list: Path):
    assert utils_model_list.exists(), (
        f"Expected output file {utils_model_list} not found."
    )
