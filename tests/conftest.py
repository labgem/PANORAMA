...
def pytest_addoption(parser):
    ...
    parser.addoption(
        ...
        action="store",
        default=None,
        help="Path to test dataset repository. Can also be set via PANORAMA_TEST_DATA_PATH environment variable. When provided, functional tests will be executed.",
    )
...
def test_data_path(request):
    ...
    # Convert to Path object and validate that it exists
    path = Path(path_str)
    if not path.exists():
        warnings.warn(
...
def pytest_collection_modifyitems(config, items):
    ...
    test_data_path_str = config.getoption("--test-data-path") or os.environ.get("PANORAMA_TEST_DATA_PATH")
    logger.info(f"Using test data path: '{test_data_path_str}'")
    has_valid_test_data = test_data_path_str and Path(test_data_path_str).exists()
    # Skip tests that require test data if no valid test data path is available
    if not has_valid_test_data:
