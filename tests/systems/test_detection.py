...
from panorama.systems.detection import (get_functional_unit_gene_families, 
                                        ...
                                        check_for_needed_units, get_system_unit_combinations)
...
def test_get_functional_unit_gene_families_simple_case(multi_unit_model, simple_gf2fam):
    ...
    assert get_functional_unit_gene_families(fu1, gfs, simple_gf2fam) == (set(gfs[:6]), {gfs[-1]}) 


def test_get_functional_unit_gene_families_exclude_non_pangenome_fams(multi_unit_model, simple_gf2fam):
    ...
    assert get_functional_unit_gene_families(fu2, gfs, simple_gf2fam) == (set(gfs[6:9]), set()) # GF10 and 11 excluded as they are not in the pangenome (not in gf2fam)


def test_search_unit_in_cc(multi_unit_model, simple_gf2fam, simple_fam2source, simple_pangenome): 
    ...
    detected_units = set()
    gfs = list(simple_gf2fam.keys())
    comb_families = set(gfs[:6]) # a combination that is subset of (mandatory | accessory) families of fu1 AND found in a connected component of the context graph
    ...
    assert search_unit_in_cc(filtered_cc, comb_families, fu, 'source1', simple_gf2fam, simple_fam2source, detected_units) == {frozenset(gfs[:4])} 
...
def test_search_unit_in_combination(multi_unit_model, simple_gf2fam, simple_fam2source, simple_matrix, simple_pangenome):
    ...
    fu = multi_unit_model.get('fu1')
    gfs = list(simple_gf2fam.keys())
    families_in_cc = set(gfs[:6]) # (mandatory | accessory) families of fu1 found in a connected component of the context graph
    ...
    combinations_in_cc = list({frozenset(gfs[:4])}) # list of all combinations subset of families_in_cc
    combinations2orgs = defaultdict(set, {frozenset(gfs[:4]):  # dict of all comb-to-orgs associations returned by compute_gene_context_graph of ppanggolin
                                        {'org1', 'org2'}})  # used for local filtering
    detected = search_unit_in_combination(cc, families_in_cc, simple_gf2fam, simple_fam2source, fu, 'source1', simple_matrix, 
...
def test_search_unit_in_context(multi_unit_model, simple_gf2fam, simple_fam2source, simple_matrix, simple_pangenome):
    '''Tests that the function correctly identifies units given the full context graph'''
    detected = set()
    gfs = list(simple_gf2fam.keys())
    fu_families = set(gfs[:6]) # (mandatory | accessory) families of fu1 that are present in the pangenome
    ...
    #                 GF6 -- GF7 -- GF8
    detected = search_unit_in_context(context_graph, fu_families,  simple_gf2fam, simple_fam2source, simple_matrix, func_unit, 'source1', combinations2orgs, local=True)
    assert set(next(iter(detected)).families) == set(gfs[:4]) # detected unit corresponding to the combination in combinations2orgs


def test_search_system_units(multi_unit_model, simple_gf2fam, simple_fam2source, simple_pangenome):
    ...
    detected_systems = search_system_units(multi_unit_model, model_families, simple_gf2fam, simple_fam2source, source='source1')
    ...
    assert set(next(iter(detected_systems['fu1'])).families) == set(gfs[:7]) # GF6 detected as context family (since transitivity = window = 1)
    assert set(next(iter(detected_systems['fu1'])).models_families) == set(gfs[:6]) # model families of fu1
    
    # Increment window size of fu1
    multi_unit_model.get('fu1').window = 2
    detected_systems = search_system_units(multi_unit_model, model_families, simple_gf2fam, simple_fam2source, source='source1')
    assert set(next(iter(detected_systems['fu1'])).families) == set(gfs[:8]) # GF7 is now detected as context family
    assert set(next(iter(detected_systems['fu1'])).models_families) == set(gfs[:6]) # model families of fu1 remain the same


def test_check_for_needed_units(simple_gfs, multi_unit_model):
    '''Tests that the function correctly checks if the detected functional units satisfy the model requirements.'''
    gf2meta_info = {gf: ('source1', 1) for gf in simple_gfs[:6]} # dict of GFs-to-metadata for the GFs of one detected unit
    su = SystemUnit(functional_unit=multi_unit_model.get('fu1'), source='source1', # a detected system unit
...
def test_get_system_unit_combinations(simple_gfs, multi_unit_model):
    ...
    gf2meta_info = {gf: ('source1', 1) for gf in simple_gfs[:6]} # dict of GFs-to-metadata for the GFs of one detected unit
    su1 = SystemUnit(functional_unit=multi_unit_model.get('fu1'), source='source1',
                gene_families=set(simple_gfs[:6]), families_to_metainfo=gf2meta_info)
    ...
    su2 = SystemUnit(functional_unit=multi_unit_model.get('fu2'), source='source1',
    ...
    assert [set(c) for c in get_system_unit_combinations(detected_units, multi_unit_model)] == [{su1},{su1, su2}] # convert combs to sets to avoid order discrepancies
    
    # Let fu2 be mandatory => model only satisfied with both su and su2
    multi_unit_model.mandatory.add(multi_unit_model.get('fu2'))
    multi_unit_model.accessory.remove(multi_unit_model.get('fu2'))
    multi_unit_model.min_mandatory = 2
    assert [set(c) for c in get_system_unit_combinations(detected_units, multi_unit_model)] == [{su1, su2}] 


def test_search_system(multi_unit_model, simple_gf2fam, simple_fam2source, simple_pangenome):
    ...
    detected_system = next(iter(search_system(multi_unit_model, model_fams, simple_gf2fam, simple_fam2source, 'source1'))) # extract the first detected system
    expected_unit = SystemUnit(functional_unit=multi_unit_model.get('fu1'), source='source1', 
                            gene_families=set(gfs[:7]), # GF6 does not have corresponding metainfo => context family
    ...
    assert set(expected_unit.models_families) == set(gfs[:6]) # ensure models_families is as expected
    assert set(next(iter((set(detected_system.units)))).families) == set(expected_unit.families) == set(gfs[:7]) # unit families include GF6 as well
...
def test_search_systems(simple_gfs, multi_unit_model, simple_pangenome):
    ...
    expected_units = {SystemUnit(functional_unit=multi_unit_model.get('fu1'), source='source1',
