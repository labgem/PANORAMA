...
from panorama.systems.systems_projection import (unit_projection, has_short_path,
                                                 compute_genes_graph, compute_gene_components,
                                                 project_unit_on_organisms, system_projection,
                                                 project_pangenome_systems, get_org_df,
...
def test_compute_genes_graph(simple_gfs, simple_fu, simple_orgs, simple_pangenome):
    ...
    '''
    gf2meta_info = {gf: ('source1', 1) for gf in simple_gfs[:4]} # dict of GFs-to-metadata for the GFs of one detected unit
    unit = SystemUnit(functional_unit=simple_fu, source='source1',
                      gene_families=set(simple_gfs[:4]), families_to_metainfo=gf2meta_info)
    org_unit_genes = {gf[f'gene_{i}_0'] for i, gf in enumerate(simple_gfs[:4])}  # genes of the unit model families present in the organism
                       
    genes_graph = compute_genes_graph(org_unit_genes, unit)
    
    assert org_unit_genes == {gf[f'gene_{i}_0'] for i, gf in enumerate(simple_gfs[:4])} # ensure correct genes are included
    assert set(genes_graph.nodes) == {gf[f'gene_{i}_0'] for i, gf in enumerate(simple_gfs[:6])}

    G = nx.Graph()
    expected_edges = [
    (simple_gfs[0][f'gene_0_0'], simple_gfs[1][f'gene_1_0'], {'transitivity': 0}), # 0 -> 1
    (simple_gfs[0][f'gene_0_0'], simple_gfs[2][f'gene_2_0'], {'transitivity': 1}), # 0 -> 2
    (simple_gfs[1][f'gene_1_0'], simple_gfs[2][f'gene_2_0'], {'transitivity': 0}), # 1 -> 2
    (simple_gfs[1][f'gene_1_0'], simple_gfs[3][f'gene_3_0'], {'transitivity': 1}), # 1 -> 3
    (simple_gfs[2][f'gene_2_0'], simple_gfs[3][f'gene_3_0'], {'transitivity': 0}), # 2 -> 3
    (simple_gfs[2][f'gene_2_0'], simple_gfs[4][f'gene_4_0'], {'transitivity': 1}), # 2 -> 4
    (simple_gfs[3][f'gene_3_0'], simple_gfs[4][f'gene_4_0'], {'transitivity': 0}), # 3 -> 4
    (simple_gfs[3][f'gene_3_0'], simple_gfs[5][f'gene_5_0'], {'transitivity': 1}), # 3 -> 5
    (simple_gfs[4][f'gene_4_0'], simple_gfs[5][f'gene_5_0'], {'transitivity': 0})  # 4 -> 5
    ]
...
def test_compute_gene_components(simple_gfs):
    ...
    '''
    model_genes = {gf[f'gene_{i}_0'] for i, gf in enumerate(simple_gfs[:4])} # genes of the unit model families present in the organism
    window_size = 2
    assert compute_gene_components(model_genes, window_size) == [[gf[f'gene_{i}_0'] for i, gf in enumerate(simple_gfs[:6])]]
...
def test_project_unit_on_organisms(simple_gfs, simple_fu, simple_pangenome):
    ...
    unit = SystemUnit(functional_unit=simple_fu, source='source1',
            gene_families=set(simple_gfs[:6]), families_to_metainfo={gf: ('source1', 1) for gf in simple_gfs[:4]})
    ...
    expected = [
        ['fu', '1', 'org_0', 'GF0', 'persistent', 'protein0', '', 'gene_0_0', '', 'contig_0', '100', '200', '+', 'False', 'model', 'strict', '1.0', ''],
        ['fu', '1', 'org_0', 'GF1', 'persistent', 'protein1', '', 'gene_1_0', '', 'contig_0', '200', '300', '+', 'False', 'model', 'strict', '1.0', ''],
        ['fu', '1', 'org_0', 'GF2', 'persistent', 'protein2', '', 'gene_2_0', '', 'contig_0', '300', '400', '+', 'False', 'model', 'strict', '1.0', ''],
        ['fu', '1', 'org_0', 'GF3', 'persistent', 'protein3', '', 'gene_3_0', '', 'contig_0', '400', '500', '+', 'False', 'model', 'strict', '1.0', ''],
        ['fu', '1', 'org_0', 'GF4', 'persistent', '', '', 'gene_4_0', '', 'contig_0', '500', '600', '+', 'False', 'context', 'strict', '1.0', ''],
        ['fu', '1', 'org_0', 'GF5', 'persistent', '', '', 'gene_5_0', '', 'contig_0', '600', '700', '+', 'False', 'context', 'strict', '1.0', '']
    ]
...
def test_unit_projection(simple_gfs, simple_fu, simple_gf2fam, simple_pangenome):
    ...
    unit = SystemUnit(functional_unit=simple_fu, source='source1',
                      gene_families=set(simple_gfs[:6]), families_to_metainfo={gf: ('source1', 1) for gf in simple_gfs[:4]})
    indexes = simple_pangenome.compute_org_bitarrays() # equivalent to: indexes = {gf: i for i, gf in enumerate(simple_gfs)}
    ...
    org_proj = org_proj.sort_values(by=[2, 3]).reset_index(drop=True)  # Sort by organism, then gene family
...
def test_system_projection(simple_gfs, single_unit_model, simple_gf2fam, simple_pangenome):
    ...
    unit = SystemUnit(functional_unit=single_unit_model.get('fu'), source='source1',
                      gene_families=set(simple_gfs[:6]), families_to_metainfo={gf: ('source1', 1) for gf in simple_gfs[:4]})
...
def test_project_pangenome_systems(simple_gfs, single_unit_model, simple_pangenome):
    ...
    org_proj = org_proj.sort_values(by=['organism', 'gene family']).reset_index(drop=True)  # Sort by organism, then gene family
...
def test_get_org_df():
    ...
    'annotation': ['protein0', 'protein1', 'protein2', 'protein3', '', ''],
    'secondary_names': [''] * 6,
    'gene.ID': ['gene_0_0', 'gene_1_0', 'gene_2_0', 'gene_3_0', 'gene_4_0', 'gene_5_0'],
    'gene.name': [''] * 6,
    'contig': ['contig_0'] * 6,
