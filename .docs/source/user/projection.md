...
1. **System Requirements**: Projection respects model-defined requirements for family co-occurrence and genomic
   organization
2. **Window Size**: Genomic context is determined by the `window` parameter in the functional unit definition
3. **Strand Sensitivity**: Some systems may require genes on the same strand (`same_strand` parameter)
4. **Memory Usage**: Large pangenomes may require substantial memory for parallel processing
5. **Output Size**: Detailed organism files can be large for pangenomes with many systems

## 🐛 Troubleshooting

### Common Issues

- **Empty Results**: Check that systems were detected (`panorama detect`) and sources match
- **Memory Errors**: Reduce `--threads` or process smaller subsets of organisms
- **Missing Associations**: Ensure RGPs/spots were computed if using `--association`
- **Slow Performance**: Use more threads and ensure adequate RAM availability